...
int main (int argc, char * const argv[]) {
	...
	*/
	...
	char filename_modified[100];
	int modified_counter = 0;
	for (int i = 0; i < 80; i++)
	{
		filename_modified[i + modified_counter] = filename[i];
		if (filename[i] == '\\')
		{
			modified_counter++;
			filename_modified[i + modified_counter] = '\\';
		}
	}
	...
	// basic info
	fprintf(out, "\"filename\":\"%s\",\"width\":%d,\"height\":%d", filename_modified, bitmap->width, bitmap->height);
	...
	for (int idx = 0; idx < palette->numColors; idx++) {
		...
		if (idx) fprintf(out, ",");
		fprintf(out, "{\"R\":%d,\"G\":%d,\"B\":%d}", color.red, color.green, color.blue);
	}
	fprintf(out, "]");
